...
export default function InfoCard({ title, category, description }: InfoCardObject) {
  return (
    <div className="min-h-32 border-primary border-1 m-1 lg:m-0 lg:border-none lg:m-none group aspect-square cursor-default hover:bg-primary hover:text-white text-start justify-between flex-1 p-4 lg:p-[64px] flex items-start flex-col transition-colors duration-300">
      <h4 className="uppercase group-hover:text-white text-primary-light tracking-[1.5px]">{category}</h4>
      <h3 className="mt-4 lg:mt-[50px] max-w-[328px]">{title}</h3>
      <p className="transition-colors duration-300 group-hover:!text-white mt-[16px] max-w-[328px] ">
        {description}
      </p>
      <div className="lg:mt-[80px] flex items-center justify-center h-8 w-8 lg:h-[50px] lg:w-[50px] border">
